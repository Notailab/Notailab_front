<template>
  <div class="h-screen flex flex-col overflow-hidden bg-[#f6f8f5] text-slate-900 relative">
    <SiteHeader
    ...
    <div class="flex flex-1 min-h-0 overflow-hidden">
      <aside class="flex w-56 flex-col overflow-hidden border border-slate-200 bg-white shadow-sm">
        <div class="p-4">
          <button
            class="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 py-2.5 text-sm text-white shadow-sm transition hover:bg-emerald-800"
            @click="handleNewFile"
          >
      ...
      <div class="flex flex-1 min-h-0 flex-col overflow-hidden">
        <div class="mb-3 shrink-0 flex items-center justify-between border bg-white px-4 py-2 shadow-sm lg:px-5">
          <div class="flex items-center">
            <button
              v-if="currentFile?.file_id"
              @click="handleDeleteFile"
              class="flex items-center justify-center rounded-xl bg-slate-100 px-3 py-2 text-sm text-slate-600 transition hover:bg-slate-200 hover:text-red-500"
              title="删除当前文件"
            >
        ...
        <div class="flex-1 min-h-0 overflow-hidden">
          ...
          <div v-else class="flex h-full min-h-0 items-stretch gap-0 overflow-hidden" id="drag-container">
            ...
            <div class="flex h-full min-h-0 flex-1 flex-col overflow-hidden border bg-white shadow-sm">
              <div class="shrink-0 flex items-center justify-between border-b bg-emerald-50 px-4 py-3">
              ...
              <div class="flex-1 min-h-0 space-y-4 overflow-y-auto p-4">
                <div v-for="(msg, idx) in messages" :key="idx" class="flex flex-col" :class="msg.role === 'user' ? 'items-end' : 'items-start'">
                  <div
                    class="max-w-[85%] rounded-md px-3 py-2 text-sm"
                    :class="msg.role === 'user' ? 'rounded-br-none bg-emerald-600 text-white' : 'rounded-bl-none bg-slate-100 text-slate-800'"
                  >
        ...
        <div class="mt-3 shrink-0 border bg-white px-4 py-3 text-xs text-slate-500 shadow-sm lg:px-5">
          <div class="flex justify-between">
            <div class="flex items-center gap-2">
              <span class="inline-block h-2.5 w-2.5 rounded-full" :class="hasUnsavedChanges ? 'bg-red-500' : 'bg-emerald-500'"></span>
...
const editorToolbars = ['bold', 'italic', 'underline', '-', 'unorderedList', 'orderedList', '-', 'code', 'codeRow', 'link', 'image', 'table', '-', 'preview']
...
const centerEditorMaxClass = computed(() => (isPreview.value ? 'max-w-[1800px]' : 'max-w-4xl'))
...
const handleNewFile = () => {
  editorContent.value = ''
  currentFile.value = { file_id: null, name: '', content: '' }
...
const syncOpenFileFromLatestList = () => {
  ...
  currentFile.value = { ...currentFile.value, ...latestFile }
...
const handleSave = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      const createdFile = res.data || null
      if (createdFile && createdFile.file_id) {
        const normalizedFile = {
          ...createdFile,
          name: createdFile.name || normalizedName,
          content: createdFile.content || editorContent.value || '',
        }
        fileList.value = [
          ...fileList.value.filter((file) => file.file_id !== normalizedFile.file_id),
          normalizedFile,
        ]
        openFile(normalizedFile)
        editorContent.value = normalizedFile.content || ''
